...
<script type="text/javascript">
  ...
  var btnCust = '';
  @if (!empty($field['value']))
    {{-- Files are publically accessible via the storage symlink, images are not. So, the latter is routed through the image/download route --}}
    @if ($field['type'] == 'image')
    btnCust = '<a href="{{ route('ctrl::image',['download',$field['value']]) }}" class="btn btn-default" id="fileinput-custom-download_{{ $field['id'] }}"><i class="fa fa-download"></i> Download</a>';
    @elseif ($field['type'] == 'file')
    btnCust = '<a href="{{ asset("storage/".$field['value']) }}" class="btn btn-default" id="fileinput-custom-download_{{ $field['id'] }}"><i class="fa fa-download"></i> Download</a>';
    @endif
  @endif
  ...
  $('#{{ $field['id'] }}_krajee').fileinput({
    ...
    @if (!empty($field['value']))
      @if ($field['type'] == 'image')
  	  	initialPreview: [
  		    '<img src="{{ route('ctrl::image',['view',$field['value']]) }}" class="file-preview-image">'
  		],
      /* I don't think we should preview files (as opposed to images). The one it gives you after upload is really impressive, but I can't see how to include this when the form is first loaded. I suspect you can't (it'd be a security risk, surely?) */
      @endif
